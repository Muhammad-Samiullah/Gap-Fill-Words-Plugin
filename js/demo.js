var gapText, dragWordId;
document.querySelector('.feedback-incorrect').style.display = 'none';
document.querySelector('.drags-holder').style.display = 'none';
document.querySelector('#actions-gap').style.display = 'none';

document.addEventListener('DOMContentLoaded', function() {
	if(document.querySelector('.gap-fill')) {
		document.getElementById('dragAndDrop1').style.display = 'block';
		document.querySelector('.drags-holder').style.display = 'block';
		document.querySelector('#actions-gap').style.display = 'block';
		gapText = document.querySelector('.gap-fill').innerText;
		boldText = document.querySelectorAll('.gap-fill strong');
		let para = document.createElement('p');
		var select = '';
		let selectList = [];
		let count = 1;
		boldText.forEach(boldW => {
			select = '';
			let currentText = boldW.innerText;
			gapText = gapText.replace(boldW.innerText, '<span class="wjec-drop ui-droppable"></span>haha');
			select += '<select class="dropdown dropdown-'+count+'"><option selected="true" disabled="disabled">Select your option</option>';
			count += 1;
			boldText.forEach(bold => {
				if (bold.innerText == currentText) {
					select += '<option value="correct">'+bold.innerHTML+'</option>';
				}
				else {
					select += '<option value="incorrect">'+bold.innerHTML+'</option>';
				}
			})
			select += '</select>';
			selectList.push(select);
			var boldWord = document.createElement('span');
			boldWord.innerText = boldW.innerText;
			boldWord.setAttribute('class', 'wjec-drag ui-draggable ui-draggable-handle');
			boldWord.setAttribute('style', 'position: relative');
			document.querySelector('.drags-holder').append(boldWord);
		})
		selectList.forEach(selectElement => {
			gapText = gapText.replace('haha', selectElement);
		})
		para.innerHTML = gapText;
		document.querySelector('.wjec-drag-and-drop-text').append(para);
	}
})
...
//         showFeedback(totalCorrect);
    ...
    $.fn.dragAndDropWords = function(options) {
          ...
          /*
           * Anything with a class of wjec-drop gets added as a drag
           */
...
//                   dragsHolder.append('<span class="wjec-drag">' + dropText + '<i class="fa fa-times"></i><i class="fa fa-check"></i></span>');
              ...
              //set all drop zone to the width of the longest word
          ...
          /*
           * @param {DOM Elements} - all drag items
           */
...
//               var totalCorrect = checkDropdown();
              let spans = document.querySelectorAll('.wjec-drag');
              let dropDowns = document.querySelectorAll('.dropdown')
			  let totalCorrect = 0;
			  let spanText = '.';
	 			dropDowns.forEach((dropDown, index) => {
					spanText = '.';
					if (dropDown.previousElementSibling.firstElementChild) {
						spanText = dropDown.previousElementSibling.firstElementChild.innerHTML;
					}
		 			var children = dropDown.childNodes;
					children.forEach(child => {
						if (child.getAttribute('value') == 'correct' && child.innerText == spanText) {
							totalCorrect += 1;
						}
					})
	 			})
